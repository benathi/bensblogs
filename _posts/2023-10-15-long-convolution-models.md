...
There are recent exciting developments of AI models for long context involving the use of convolutions, starting from State Space Models and its efficient version S4 (..), or a simpler variant Gated State Spaces (GSS), or Hungry Hungry Hippos (H3), and most recently, Hyena Models. In this blog, we will seek to develop intuition behind these models with some illustrations. We will aim for this blog post to be as accessible as possible, which means we will trade mathematical rigor for intuitive explanation and may omit some details related to the convergence conditions etc. (after all, if you want mathematical regor, the corresponding original papers also do a great job).
...
### Dense Orthonormal Basis in Function Space
...
### (2) Background: Convolution and Fast Fourier Transform
...
Let us consider two dimensional vectors $$\mathbf{a}, \mathbf{b} \in \mathbb{R}^N$$. The convolution of the two vectors, denoted as $$\mathbf{a} * \mathbf{b}$$ is defined as:
...
### Background (3) -- State Space Models
